...
mock_approvals: List[Dict[str, Any]] = [
    ...
    },
    {
        ...
        "createdBy": "grading-agent",
        "assignedTo": "professor-1",
        "metadata": {
            "student": "student-123",
...
async def get_pending_approvals(
    ...
    filtered_items = [
        item for item in mock_approvals
        if item["status"] == "pending" and
        (not type or item["type"] == type) and
        (not priority or item["priority"] == priority) and
        (not assignedTo or item["assignedTo"] == assignedTo)
    ]
...
async def approve_item(approval_id: str, comment: ApprovalComment) -> Dict[str, Any]:
    ...
    item["approvedAt"] = datetime.utcnow().isoformat() + "Z"
    item["approverComment"] = comment.comment
...
async def reject_item(approval_id: str, rejection: ApprovalRejection) -> Dict[str, Any]:
    ...
    item["rejectedAt"] = datetime.utcnow().isoformat() + "Z"
    item["rejectionReason"] = rejection.reason
...
async def edit_item(approval_id: str, updates: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key, value in updates.items():
        if key in item and key not in ["id", "createdAt", "createdBy"]:
            item[key] = value
